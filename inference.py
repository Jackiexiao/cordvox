...
for i, path in enumerate(paths):
    ...
    with torch.no_grad():
        ...
        for chunk in chunks:
            ...
            chunk = G(log_mel(chunk), args.noise, args.harmonics)
    ...
    plot_spec(log_mel(wf_in), os.path.join("./outputs/", f"{file_name}_input.png"))
    plot_spec(log_mel(wf_out), os.path.join("./outputs/", f"{file_name}_output.png"))
    torchaudio.save(os.path.join("./outputs/", f"{file_name}.wav"), src=wf, sample_rate=sr)
